...
def main():
    parser = argparse.ArgumentParser(description="Train 3D Swin-UNETR on GI Tract Segmentation")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_data in train_loader:
            ...
            is_gold_strong = batch_data.get("is_gold_standard", False)
            # Handle case where is_gold_standard might be a tensor or batch
            if isinstance(is_gold_strong, torch.Tensor):
                ...
                is_gold_any = is_gold_strong.any().item()
            else:
                is_gold_any = bool(is_gold_strong)
